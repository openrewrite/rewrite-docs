...
_Using upper case literal suffixes for declaring literals is less ambiguous, e.g., `1l` versus `1L`. A lowercase `l` is easily mistaken for the digit `1` in many fonts, which can lead to incorrect assumptions about the value. Hexadecimal digits are upper cased as well, e.g., `0Xabc` versus `0xABC`, such that they stand out from the lower case `0x` prefix and `p` exponent._
