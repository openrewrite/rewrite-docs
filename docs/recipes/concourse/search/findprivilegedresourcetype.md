...
This recipe has no required configuration options. It can be activated by adding a dependency on `org.openrewrite.recipe:rewrite-concourse:{{VERSION_REWRITE_CONCOURSE}}` in your build file or by running a shell command (in which case no build changes are needed): 
