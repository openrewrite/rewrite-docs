...
This recipe has no required configuration options. It can be activated by adding a dependency on `org.openrewrite.recipe:rewrite-sql:{{VERSION_REWRITE_SQL}}` in your build file or by running a shell command (in which case no build changes are needed): 
...
dependencies {
    rewrite("org.openrewrite.recipe:rewrite-sql:{{VERSION_REWRITE_SQL}}")
}
```
      ...
      <plugin>
        ...
        <dependencies>
          <dependency>
            ...
            <version>{{VERSION_REWRITE_SQL}}</version>
          </dependency>
        </dependencies>
      </plugin>
    </plugins>
