...
This recipe has no required configuration options. It can be activated by adding a dependency on `org.openrewrite.recipe:rewrite-reactive-streams:0.3.0` in your build file or by running a shell command (in which case no build changes are needed): 
...
dependencies {
    rewrite("org.openrewrite.recipe:rewrite-reactive-streams:0.3.0")
}
```
