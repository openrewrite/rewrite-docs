...
* [Replace `ReplayProcessor.cacheLast()` with `Sinks.many().replay().latest()`](../../reactive/reactor/reactorprocessorcachetosinkrecipes$replayprocessorcachetosinkrecipe)
* [Replace `ReplayProcessor.cacheLast()` with `Sinks.many().replay().latest()`](../../reactive/reactor/reactorprocessorcachetosinkrecipes$replayprocessorcachedefaulttosinkrecipe)
...
This recipe has no required configuration options. It can be activated by adding a dependency on `org.openrewrite.recipe:rewrite-reactive-streams:0.3.1` in your build file or by running a shell command (in which case no build changes are needed): 
...
dependencies {
    rewrite("org.openrewrite.recipe:rewrite-reactive-streams:0.3.1")
}
```
...
<a href="https://app.moderne.io/recipes/org.openrewrite.reactive.reactor.ReactorProcessorCacheToSinkRecipes">
    <img
