...
_Removes the `cucumber-java8` dependency where every `LambdaGlue` call migrates to `cucumber-java`, and retains it wherever one is left behind. Read from the glue as it stands before the migration, as what the migration leaves behind only becomes visible to a scanning recipe in the cycle after, which a build tool run never reaches. Glue left anywhere retains the dependency everywhere, an unused dependency being the one outcome here that still compiles._

## Recipe source

[GitHub: RemoveCucumberJava8Dependency.java](https://github.com/openrewrite/rewrite-cucumber-jvm/blob/main/src/main/java/org/openrewrite/cucumber/jvm/RemoveCucumberJava8Dependency.java),
...
<RunRecipe
  recipeName="org.openrewrite.cucumber.jvm.RemoveCucumberJava8Dependency"
...
<RecipeCallout link="https://app.moderne.io/recipes/org.openrewrite.cucumber.jvm.RemoveCucumberJava8Dependency" />
