...
* [Detect async callbacks in synchronous array methods](/recipes/javascript/cleanup/async-callback-in-sync-array-method.md) - _Detects async callbacks passed to array methods like .some(), .every(), .filter() which don't await promises. This is a common bug where Promise objects are always truthy._
...
* [Replace deprecated `node:_tls_common` and `node:_tls_wrap` with `node:tls`](/recipes/node/migrate/tls/replace-internal-modules.md) - _Replace deprecated internal TLS module imports `require('node:_tls_common')` and `require('node:_tls_wrap')` with the public `node:tls` module._
...
* [Update configuration of serialization inclusion in ObjectMapper for Jackson 3](/recipes/java/jackson/updateserializationinclusionconfiguration.md) - _In Jackson 3, `mapper.setSerializationInclusion(JsonInclude.Include.NON_NULL)` is no longer supported and should be replaced by `changeDefaultPropertyInclusion()` for both `valueInclusion` and `contentInclusion`._
