...
| `org.Foo bar(java.*.*, *)`                  | Exactly the `bar` method in the `org.Foo` class that takes exactly 2 arguments of which the first type is nested exactly 2 levels deep in the `java` package.                                                      |
