...
* `recipeArtifactCoordinates` - Alternative way to specify recipe dependencies as Maven coordinates. Can be used instead of or in addition to plugin dependencies. For example, `-Drewrite.recipeArtifactCoordinates=org.openrewrite.recipe:rewrite-java-dependencies:1.0.0`. These artifacts are resolved from the project's repositories, so the [Code Genome Project repository](/running-recipes/running-rewrite-on-a-maven-project-without-modifying-the-build#configure-the-code-genome-project-repository) has to be declared as a `<repository>` in either your `pom.xml` or your `settings.xml`; declaring it only as a `<pluginRepository>` is not enough.
