...
OpenRewrite works by making changes to [Lossless Semantic Trees](broken-reference) (LST) representing your source code and printing the modified trees back into source code. You can then review the changes in your code and commit. Modifications to the LST are performed in [Visitors](broken-reference) and visitors are aggregated into [Recipes](broken-reference). OpenRewrite recipes make minimally invasive changes to your source code that honor the original formatting.
...
* If you want to learn how to use OpenRewrite to perform code transformations in a Maven or Gradle project, check out the [Setting Up Your Project and Running Recipes Guide](broken-reference).
* If you want to learn how to create your own recipes, start with the [Recipe Development Environment guide](broken-reference) and then work through the [Writing a Java Refactoring Recipe guide](broken-reference).
