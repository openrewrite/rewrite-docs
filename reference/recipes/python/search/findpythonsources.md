...
[GitHub](https://github.com/openrewrite/rewrite-python/blob/main/src/main/java/org/openrewrite/python/search/FindPythonSources.java), [Issue Tracker](https://github.com/openrewrite/rewrite-python/issues), [Maven Central](https://central.sonatype.com/artifact/org.openrewrite/rewrite-python/1.2.0/jar)
...
This recipe has no required configuration options. It can be activated by adding a dependency on `org.openrewrite:rewrite-python:1.2.0` in your build file or by running a shell command (in which case no build changes are needed): 
{% tabs %}
{% tab title="Gradle" %}
...
dependencies {
    rewrite("org.openrewrite:rewrite-python:1.2.0")
}
```
{% endcode %}
...
{% code title="shell" %}
```shell
mod run . --recipe FindPythonSources
```
{% endcode %}
{% endtab %}
{% endtabs %}
...
## Contributors
[traceyyoshima](mailto:tracey.yoshima@gmail.com), [Knut Wannheden](mailto:knut@moderne.io), Mike Sol
