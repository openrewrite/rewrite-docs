...
_Removes the `Enclosed` specification from a class, and adds `Nested` to its inner classes_
...
mvn org.openrewrite.maven:rewrite-maven-plugin:4.33.0:run \
  ...
  -DactiveRecipes=org.openrewrite.java.testing.junit5.EnclosedToNested
```
{% endcode %}
{% endtab %}
{% endtabs %}

Recipes can also be activated directly from the command line by adding the argument `-Drewrite.activeRecipes=org.openrewrite.java.testing.junit5.EnclosedToNested`
