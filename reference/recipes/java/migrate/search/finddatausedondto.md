...
This recipe has required configuration parameters. Recipes with required configuration parameters cannot be activated directly. To activate this recipe you must create a new recipe which fills in the required parameters. In your `rewrite.yml` create a new recipe with a unique name. For example: `com.yourorg.FindDataUsedOnDtoExample`.
...
Now that `com.yourorg.FindDataUsedOnDtoExample` has been defined activate it and take a dependency on org.openrewrite.recipe:rewrite-migrate-java:2.20.0 in your build file:
{% tabs %}
{% tab title="Gradle" %}
...
{% code title="shell" %}
```shell
mod run . --recipe FindDataUsedOnDtoExample
```
{% endcode %}
{% endtab %}
{% endtabs %}
...
[![Moderne Link Image](/.gitbook/assets/ModerneRecipeButton.png)](https://app.moderne.io/recipes/org.openrewrite.java.migrate.search.FindDataUsedOnDto)
