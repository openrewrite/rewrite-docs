...
Now that `com.yourorg.UseTlsJdbcConnectionStringExample` has been defined activate it and take a dependency on org.openrewrite.recipe:rewrite-spring:5.2.0 in your build file:
{% tabs %}
{% tab title="Gradle" %}
...
{% code title="shell" %}
```shell
mod run . --recipe UseTlsJdbcConnectionString
```
{% endcode %}
{% endtab %}
{% endtabs %}
