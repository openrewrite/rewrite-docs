...
At this point you're able to run any of the Maven goals or Gradle tasks provided by the plugins. See [Maven Plugin Configuration](reference-1/rewrite-maven-plugin.md) and [Gradle Plugin Configuration](reference-1/gradle-plugin-configuration.md) for the full set of options. Try running `mvn rewrite:discover` or `gradlew rewriteDiscover` to see a listing of all the recipes available for execution. Until we add dependencies on recipe-providing modules this will list only the recipes built-in to openrewrite.
